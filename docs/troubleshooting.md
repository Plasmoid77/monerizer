...
Первый шаг всегда `moneroid doctor`, второй — `moneroid logs <target>`. Moneroid показывает следствия и коды причин, но не угадывает причину за upstream (ТЗ DOC-04).
...
| `UNIT_FAILED`, `Result=start-limit-hit` | Служба падала 5 раз за минуту | `moneroid logs`; чаще всего неверный `wallet` или недоступная нода; после исправления `systemctl reset-failed` не нужен — `moneroid start` |
| `UNIT_MASKED` | Unit замаскирован | `systemctl unmask` |
| `UNIT_NOT_ACTIVE` (health degraded) | Работает только одна служба | `moneroid start <вторая>` |
...
| `SOURCE_ID_MISMATCH` | На порту другой XMRig | `api.id` в `xmrig.json` = `expected_id` в `moneroid.toml` |
| `PERMISSION_DENIED` для `p2pool_*` | Оператор не в группе `moneroid` или не перелогинился | `usermod -aG moneroid`, новый вход |
...
| `SOURCE_STALE` (`local/p2p` старше 180 s) | P2Pool завис или не пишет API | `moneroid logs p2pool`, `restart p2pool` |
...
| `ZMQ_ACTIVITY_OLD` | >600 s без ZMQ-событий | Нода без `--zmq-pub`, неверный `zmq-port` или нестабильный ZMQ у удалённой ноды (в журнале повторяющиеся `ZMQReader disconnected`) → `moneroid node list`, выбрать другую |
| `SIDECHAIN_BEHIND` | Локальная sidechain ниже высоты peers | Первые ~10 минут после старта — норма (P2Pool качает и проверяет окно, в журнале растут `verified block`); если не догоняет — `moneroid logs p2pool`, нода/сеть |
...
| `moneroid start`: код 3 | Нет прав на systemd | `sudo` или polkit-правило |
| `moneroid start`: код 4 | systemctl не ответил за 90 s | Задание могло продолжиться: `moneroid status` |
...
Источник прокси — любой хост с чистым интернетом, например `ssh -N -D 127.0.0.1:1080 user@host` (ключ на той стороне ограничьте `restrict,port-forwarding`), или VPN-клиент на самом хосте. `moneroid node list/select` и `doctor` при заданном `socks5` пробуют ноды через тот же прокси. Кэш изолированной цепочки после исправления сети лучше удалить: `moneroid stop p2pool; rm /var/lib/moneroid/p2pool/p2pool.cache; moneroid start p2pool`.
