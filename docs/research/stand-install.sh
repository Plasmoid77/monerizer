...
[ -d /etc/polkit-1/rules.d ] && install -o root -g root -m 0644 "$REPO/examples/polkit/50-monerizer.rules" /etc/polkit-1/rules.d/50-monerizer.rules || echo "polkit rules.d absent (polkit < 0.106): use sudo for control"
