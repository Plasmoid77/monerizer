...
func TestLoadGood(t *testing.T) {
	...
	if c.UI.RefreshMs != 1000 || c.Services.P2Pool != "monerizer-p2pool.service" {
...
func TestLoadBad(t *testing.T) {
	cases := map[string]string{
		...
		"same unit":      strings.Replace(good, `xmrig = "monerizer-xmrig.service"`, `xmrig = "monerizer-p2pool.service"`, 1),
		"unit glob":      strings.Replace(good, "monerizer-xmrig.service", "monerizer-*.service", 1),
		"relative path":  strings.Replace(good, "/run/monerizer-p2pool-api", "api", 1),
		...
		"empty id":       strings.Replace(good, `expected_id = "monerizer-xmrig"`, `expected_id = ""`, 1),
		"refresh low":    good + "[ui]\nrefresh_ms = 100\n",
