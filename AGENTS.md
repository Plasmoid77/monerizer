...
Monerizer — тонкий слой над двумя чужими программами (P2Pool, XMRig) и systemd. Цель владельца: **написать один раз и не сопровождать**. Отсюда главный критерий любой правки: *можно ли обойтись штатной командой systemd / journalctl / upstream?* Если да — правка не нужна.

Не добавлять: демоны, базы данных, web-UI, автообновление, скачивание бинарников, управление `monerod`, несколько стеков, учёт выплат/баланса, парсинг логов как источника метрик, форки/вендоринг upstream, новые зависимости без конкретной необходимости.
...
Не запускать майнинг на чужих или продуктивных машинах без явного разрешения владельца; после теста служб — `monerizer stop`.
...
- Фильтрующие аплинки рвут ответы > ~15 KB при живом `get_info`; P2Pool умеет `socks5`, Monerizer пробует ноды через тот же прокси.
